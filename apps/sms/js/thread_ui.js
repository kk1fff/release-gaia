...
var MessageManager = {
  init: function mm_init() {
    ...
    mmm.onsending = function(e) {
      dump("Patrick: onsending");
      dump("Patrick: id = " + e.message.id);
    };

    mmm.onsent = function(e) {
      dump("Patrick: onsent");
      dump("Patrick: id = " + e.message.id);
    };

    mmm.ondeliverysuccess = function(e) {
      dump("Patrick: ondeliverysuccess");
      dump("Patrick: id = " + e.message.id);
    };

    mmm.onreceived = function(e) {
      var msg = e.message;
      dump("Patrick: recevied. data --");
      dump("msg.id = " + msg.id);
      for (var i = 0; i < msg.attachments.length; i++) {
        dump("  attachments[" + i + "].id:           " + msg.attachments[i].id);
        dump("  attachments[" + i + "].location:     " + msg.attachments[i].location);
        dump("  attachments[" + i + "].content.type: " + msg.attachments[i].content.type);
        dump("  attachments[" + i + "].content.size: " + msg.attachments[i].content.size);
      }
      dump("-- end");
    }
  ...
  send: function mm_send(number, text, callback, errorHandler) {
    ...
    var req = mmm.sendMMS({
      receivers: [number + ""],
      subject: "Yeah, this is an MMS from firefox",
      smil: "",
      attachments: [{
        "id": "" + Date.now() + "-" + this.id++,
        "location": "Mozilla Taipei Office",
        "content": new Blob([text, text + ", again"], {type: "text/plain"})
      }, {
        "id": "" + Date.now() + "-" + this.id++,
        "location": "Mozilla Taiwan",
        "content": new Blob(["I am Patrick", "Wa ha"], {type: "text/plain"})
      }]
    });
    req.onsuccess = function onsuccess(e) {
      var msg = e.target.result;
      dump("Patrick: onsuccess data --");
      dump(" id: " + msg.id);
      for (var i = 0; i < msg.attachments.length; i++) {
        dump("  attachments[" + i + "].id:           " + msg.attachments[i].id);
        dump("  attachments[" + i + "].location:     " + msg.attachments[i].location);
